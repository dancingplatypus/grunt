...
var wrappedStatSync = function(file) {
...
var doPostProcess = function(srcFiles, dest, postProcess) {

    if (postProcess != null) {
        postProcess(srcFiles, dest);
    }
    else {
        if (srcFiles == null || srcFiles.length == 0) return;

        var fileStats = _.map(srcFiles, wrappedStatSync);
        var destStat = wrappedStatSync(dest);
        // find latest source timestamp
        var maxStat = _.reduce(fileStats, function(winner, contender) {
            console.log(contender.mtime.getTime());
            if (winner == null) return contender;
            return (winner.mtime.getTime() < contender.mtime.getTime()) ?
                winner.mtime.getTime() : contender.mtime.getTime();
        });
        fs.utimesSync(dest, maxStat.atime, maxStat.mtime);
    }
}

/*
    @param srcFiles array of file objects that represent the source files for this task
    @param destFile the destination file for this task
    @param onlyIf   'always'|'newer'|'missing'|'modified'|user defined function
*/
var doProcess = function(srcFiles, dest, onlyIf) {

    if (onlyIf == null || onlyIf === 'always') {
        return true;
    }

    if (srcFiles == null || srcFiles.length == 0) return true;

    var fileStats = _.map(srcFiles, wrappedStatSync);
    var destStat = wrappedStatSync(dest);

    var onlyIfType = Object.prototype.toString.call(onlyIf);
    if (onlyIfType === '[object Function]') {
        return onlyIf(src, dest, fileStats, destStat);
    }
    else if (onlyIfType !== '[object String]') {
        // ignore weird types
        return true;
    }
    else
    {
        // looking for 'newer', 'modified', or 'missing'
        if (onlyIf === 'missing') {
            return destStat == null;
        }

        // there are a lot of calls to mtime and getTime.  If it really causes grief,
        // we can optimize them out of there.
        ...
        if (onlyIf === 'modified') {
             return (maxStat.mtime.getTime() === destStat.mtime.getTime()) ? false : true;
        }
        ...
        // ignore any other setting, but give the user a heads up
        grunt.verbose.write("unrecognized setting for 'onlyIf' : " + onlyIf);
    }
...
function shouldProcessFile(file) {
    return doProcess(file.src, file.dest, file.onlyIf);
}
function postProcessFile(file) {
    doPostProcess(file.src, file.dest, file.postProcess);
}
...
task.registerMultiTask = function(name, info, fn) {
  ...
  task.registerTask(name, info, function(target) {
    ...
    var specifiedFiles = task.normalizeMultiTaskFiles(this.data, target);
    // filter out files that don't need to be handled
    this.files = _.filter(specifiedFiles, shouldProcessFile, this);
    ...
    var res =  fn.apply(this, this.args);

    _.each(this.files, postProcessFile, this);
